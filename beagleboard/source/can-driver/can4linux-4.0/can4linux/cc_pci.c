...
int i;
     ...
     * for the io address.
     * Here we request the io areas for excllusive use with this driver
     */
...
That said, only thre states aare possible
 LED off
 green on
 red on
...
int val;
    ...
    DBGin();
    	/* 16 green, 32 red */
    	if(argp->val1 == 1) val = 0x16;	/* green */
    	else
    	if(argp->val1 == 4) val = 0x32;	/* red */
    	else {
	    DBGout();
	    return;
	}
