 ...
 *------------------------------------------------------------------
 * $Header: /z2/cvsroot/products/0530/software/can4linux/src/mcf5282funcs.c,v 1.4 2007/06/22 08:43:53 oe Exp $
 *
 *--------------------------------------------------------------------------
 *
 *
...
int CAN_ShowStat (int minor)
{
	...
	printk(" MCR 0x%x,", CANinw(minor, canmcr));
	printk(" ESTAT 0x%x,", CANinw(minor, estat));
	printk(" IFLAGS 0x%x,", CANinw(minor, iflag));
	printk(" IMASK 0x%x,", CANinw(minor, imask));
	printk("\n");
    }
#endif
    return 0;
...
u32 reg;
    ...
    if (reg & CAN_MCR_SOFT_RST) {
       printk("Failed to softreset FlexCAN module (mcr=0x%08x)\n", reg);
               return -ENODEV;
    }
...
/* check the error counters */
	...
	&&  ((errcnt & 0xff) < 96)
	    ...
	    /* printk("can error status changed from passive to active\n"); */
	    erroractive[minor] = 1;
	...
	if (unlikely(irqsrc & (1 << 6))) {
	    /* don't take care about this, no way to tell this event
	       the application */
	    printk ("CAN[%d] RX HW FIFO Nearly Full\n", minor);
...
/* ---------- fill frame data -------------------------------- */
	...
	if (CanWaitFlag[minor][rx_fifo] == 1) {
	    ...
	    RxFifo->head = ++(RxFifo->head) % MAX_BUFSIZE;
	    if(unlikely(RxFifo->head == RxFifo->tail)) {
		    printk("CAN[%d][%d] RX: SW FIFO overrun\n", minor, rx_fifo);
		    RxFifo->status = BUF_OVERRUN;
	    } 
...
u32 reg;
    ...
    printk("Flex CAN register layout, size 0x%02x bytes\n", sizeof(flexcan_t));
...
#define  printregister(s, name) printk(s, &name , name)
    ...
    printk("Compat Mode (BCC) %s,",
    	reg & 0x10000 ? "OFF" : "ON");
    printk(" FIFO Mode (FEN) %s, IDAM = %d\n",
    ...
    printregister
    ("ControlReg               %p: %08x\n", CAN_ControlReg);
    printk("\tClock: %s,  presdiv: %d, rjw: %d, "
...
volatile u32 *cpx =
    ...
    printk("Flex CAN %d object %d", minor, object);
    printk(", at: %p: \n\t", cpx);
    for(vl = 0; vl < 4; vl++) {
	printk("%08x  ", readl(cpx + vl));
    }
    printk("\n");

    vl = CAN_OBJ[object].ctl_status;
    printk(" Ctrl/Status 0x%08x; CODE 0x%02x, l = %d \n",
