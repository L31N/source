...
u8 tx2reg, stat;
    ...
    if(tx->length > 8) { tx->length = 8; } /* limit CAN message length to 8 */
...
/* printk(KERN_INFO "> filling buffer [%d][%d]\n", minor, rx_fifo);  */
	    ...
	    if(length > 8) { 
		length = 8;	/* limit count to 8 bytes for number of data */
	    }
	    for( i = 0; i < length; i++) {
		/* SLOW_DOWN_IO;SLOW_DOWN_IO; */
		if(ext) {
...
/* printk("CAN[%d][%d] enter\n", minor, rx_fifo); */
	...
	if(tx2reg > 8) { tx2reg = 8; } 		/* restrict to 8 */
	if(ext) {
	    for( i=0; i < tx2reg ; i++) {
