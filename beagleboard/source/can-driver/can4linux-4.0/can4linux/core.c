 ...
 * (c) 2001-2012 Heinz-Jürgen Oertel (oe@port.de)
...
wher producer and consumer exchange CAN frames only virtually. 
...
/proc/sys/dev/Can/version:  3.5.9_ATCANMINI_PELICAN SVN $Revision: 134 $
\endcode
...
int minor = -1;
    ...
    */
    ...
    /*
    initialize the variables layed down in /proc/sys/dev/Can
    ========================================================
    */
    for (i = 0; i < MAX_CHANNELS; i++) {
...
int minor = -1;
  ...
  if (MOD_IN_USE) {
    ...
    printk(KERN_INFO "Can[-1]: - : char device \"" CANREGDEVNAME 					"\" successfully removed\n");
