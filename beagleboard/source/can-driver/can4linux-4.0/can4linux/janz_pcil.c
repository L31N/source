...
#if 1     /* debugging */
    printk("controller_available 0x%p\n", ptr);

    printk("%p:0x%0x, ", ptr, readb(ptr) );
    printk("%p:0x%0x, ", ptr + (2 * offset), readb(ptr + (2 * offset)) );
...
int minor = -1;			/* to make DBGin() happy */
    ...
    DBGin();
    /* JANZ CAN_PCIL
       Adressspace 0   - CAN address space
       Adressspace 2   - On board registers
     */
    ptr		= pci_iomap(pdev, 0, 128);
    cptr	= pci_iomap(pdev, 2, 2048);
    rptr	= pci_iomap(pdev, 4, 8);
...
int	can_on_board;			/* CAN controllers on one board */
	...
	&& pdev->device == PCI_DEVICE_ID_PLX_9030) {
	    /* err("found PLX 9030 pci board %d", candev); */
	    ...
	    */
		...
		err("found JANZ AG CAN board, starting with CAN%d", candev);
		if((pdev->subsystem_device && 0xFF00) == 0x1900) {
		    err(" CAN-PCIL with one CAN");
		    can_on_board = 1;
		} else if((pdev->subsystem_device & 0xFF00) == 0x1a00) {
		    err(" CAN-PCIL with two CAN");
		    can_on_board = 2;
		}
		else
